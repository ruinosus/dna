...
class PgVecRecordSearchProvider:
    ...
    async def search(
        ...
        ⚠️ Reads ONE embedding space: the table of the active width, filtered to
        the active ``model_id``. Both halves are load-bearing. Dropping the
        table routing would compare vectors of different lengths; dropping the
        ``model_id`` filter would compare vectors of the same length produced by
        different models — which the port's contract says are not comparable,
        and which no error would ever announce, because the numbers come out
        looking exactly like relevance."""
        ...
        async with pool.acquire() as conn:
            ...
            # ``model_id`` is in the predicate, not applied afterwards: a
            # post-filter would let a foreign space's vectors CROWD OUT real
            # candidates inside the LIMIT and silently shorten the result.
            dense_ranked: list[int] = []
            if any(query_vec):
                dense_rows = await conn.fetch(
                    f"SELECT id FROM {self._schema}.{table} "
