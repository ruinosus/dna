...
#
# Declarative definition of a tool an agent can invoke. Bridges the helix
# world with OpenAI/Anthropic tool-calling conventions — declares name,
# description, invocation type, input/output schemas, and auth. Stored as a
# flat yaml file under tools/<name>.yaml. Not a prompt target.
# ---------------------------------------------------------------------------

_VALID_TOOL_TYPES = {"http", "mcp", "python", "shell", "builtin"}


@dataclass
class ToolSpec:
    type: str = "builtin"                            # http | mcp | python | shell | builtin
    endpoint: str = ""                                # for http
    method: str = "POST"                              # for http
    mcp_server: str = ""                              # for mcp
    mcp_tool: str = ""                                # for mcp: tool name on the server
    python_module: str = ""                           # for python: dotted import path
    python_callable: str = ""                         # for python: attribute on module
    shell_command: str = ""                           # for shell
    ...
    auth_env_var: str = ""                            # e.g. "SERPAPI_KEY"
    read_only: bool = True                            # false = may mutate state
    requires_confirmation: bool = False               # force user confirm before call
    tags: list[str] = field(default_factory=list)
    examples: list[dict[str, Any]] = field(default_factory=list)  # [{input, output}]

    @classmethod
    def from_raw(cls, raw: dict[str, Any]) -> ToolSpec:
        t = raw.get("type", "builtin")
        if t not in _VALID_TOOL_TYPES:
            raise ValueError(
                f"Invalid Tool type: {t!r} (expected one of {sorted(_VALID_TOOL_TYPES)})"
            )
        return cls(
            type=t,
            ...
            python_callable=raw.get("python_callable", ""),
            shell_command=raw.get("shell_command", ""),
...
class TypedTool:
    ...
    @classmethod
    def from_raw(cls, raw: dict[str, Any]) -> TypedTool:
        return cls(
            metadata=Metadata.from_raw(raw.get("metadata", {})),
            spec=ToolSpec.from_raw(raw.get("spec", {})),
        )
