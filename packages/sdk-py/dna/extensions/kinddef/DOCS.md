...
brand-new kinds into a manifest without writing any Python extension code.

A KindDefinition instance lives at
`.dna/<scope>/kinds/<name>/KIND.yaml` and carries:

- `target_api_version` — the apiVersion new instances of this kind will claim
...
- `schema` — a JSON Schema that validates each instance's `spec`
- `storage` — layout descriptor (`bundle`, `yaml`, `standalone`, `root`)
- `docs` — prose description surfaced by `describe_kind`
...
that's registered on the kernel before any other instance is parsed. If a
