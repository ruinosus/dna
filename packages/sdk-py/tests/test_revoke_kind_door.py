...
* **an EDIT must not un-revoke.** The authoring door rebuilds the spec from
  ...
  instances are accepted with no validation at all. That is the same loosening
...
async def test_an_edit_cannot_un_revoke_a_kind(store: Path):
    ...
    in *never approved*, where instances are accepted with NO validation. So an
    ...
    assert approval_state(spec) == REVOKED, (
        ...
        "the Kind accept instances with no validation at all, which is the "
...
async def test_revoking_a_document_that_changed_since_it_was_read_is_refused(
    ...
    Revocation is a read-modify-write too — it reads the instance, merges two
...
async def test_the_audit_tells_revoked_apart_from_never_approved(store: Path):
    ...
    accepts instances unvalidated, the other refuses them. A roster that reports
    ...
    # drift into a second vocabulary for the same instance.
