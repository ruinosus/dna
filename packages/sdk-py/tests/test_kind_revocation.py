...
unregistered means instances are accepted with NO validation at all. So the
...
un-approval, and watched to ACCEPT a schema-violating instance.
...
async def _revoke(k: Kernel, kind: str) -> None:
    ...
    schema-violating instance be ACCEPTED.
    """
    await k.write_instance(
...
async def _seed(root, kind: str = "Widget") -> Kernel:
    ...
    instances that route to the Kind's declared container.

    The ``instance_async`` is not ceremony: registration is what confers storage
    routing, so an instance written before the Kind registers lands at the scope
...
async def _write_doc(k: Kernel, kind: str, name: str, spec: dict[str, Any]) -> None:
    await k.write_instance(_SCOPE, kind, name, {
        "apiVersion": _API,
        "kind": kind,
        "metadata": {"name": name},
        "spec": spec,
...
async def test_revoking_does_not_return_the_kind_to_accepting_anything(store):
    ...
    unregistered Kind's instances are accepted with no validation at all. So a
    revocation implemented as a plain un-approval switches the gate OFF: the
    exact instance the approved Kind refused becomes acceptable.
    """
    ...
    # Control — while APPROVED, the violating instance is refused.
    ...
    async def still_refuses_after_revocation(fresh):
        ...
        # RefusedKindWrite, not SpecValidationError: the instance's shape is no
...
def test_approval_state_is_three_states_and_revocation_is_not_absence():
    ...
    never-approved accepts instances unvalidated, revoked refuses them."""
    never = {}
...
def test_a_revoker_that_names_no_one_does_not_revoke():
    ...
    a malformed instance silently invalidate a workspace's data."""
...
async def test_an_approved_kind_validates_and_a_revoked_one_refuses_even_conforming(
    ...
    So the instance that the APPROVED Kind happily accepted is refused too.
    Asserting only on a violating instance would have let an implementation
    ...
    async def refuses_conforming(fresh):
        ...
        assert "shape" in str(exc.value), (
            "the refusal must say the instance's shape is not the subject — "
            "an author told 'validation failed' will go and edit an instance "
            "that can never pass"
        )
...
async def test_an_existing_document_reads_back_marked_never_erased(store):
    """Decision 1: the read returns THE INSTANCE, marked invalid.
    ...
    async def reads_marked(fresh):
        raw = await fresh.get_instance(_SCOPE, "Widget", "w1")
        assert raw is not None, "the instance must still be readable"
        assert raw["spec"]["size"] == "large", (
            "the instance's own content is untouched — revocation invalidates, "
...
async def test_the_document_shape_carries_the_same_verdict(store):
    ...
    ``get_instance``/``query`` hand back raw dicts, the sync wrappers and an MI
    hand back ``Instance``. A face reading one and a tool reading the other
    would otherwise reach opposite conclusions about the same instance."""
    k = await _seed(store)
    await _write_doc(k, "Widget", "w1", {"size": "large"})
    await _revoke(k, "Widget")
    ...
    async def reads_marked(fresh):
        ...
        # builder parses raw instances straight off the source, so this proves
        # the ``Instance`` shape marks them ITSELF. Feeding it rows from
...
# ── what an invalid instance does in a QUERY ──────────────────────────────
...
async def test_re_approving_restores_validity(store):
    ...
    async def is_invalid(fresh):
        return is_marked_invalid(await fresh.get_instance(_SCOPE, "Widget", "w1"))
    ...
    async def is_valid_again(fresh):
        ...
        assert not is_marked_invalid(raw), (
            "re-approving restores validity for the instances that already "
...
async def test_the_mark_is_derived_and_never_reaches_the_store(store):
    """The mark must not become a stamp on the instance.

    The application layer read-modify-writes constantly (``{**raw, "spec":
    spec}``), so a marked read handed straight back to ``write_instance`` is the
    ...
    # revoked: read the marked instance, hand it back to a write of its own
    ...
    async def round_trip(fresh):
        marked = await fresh.get_instance(_SCOPE, "Widget", "w1")
        assert is_marked_invalid(marked)
        kd = await fresh.get_instance(_SCOPE, "KindDefinition", "widget")
        await fresh.write_instance(
    ...
    async def nothing_stale(fresh):
        assert not is_marked_invalid(
            await fresh.get_instance(_SCOPE, "Widget", "w1")
        )
...
async def test_the_revocation_is_logged_loudly(store, caplog):
    """A scope whose instances have just been marked invalid is not a quiet
    ...
    assert any("Widget" in line for line in revoked_lines), (
        ...
        f"logs has to find out why a scope's instances went invalid; saw: "
...
async def test_a_revoked_kinds_documents_can_still_be_deleted(store):
    """Refusing deletes would trap exactly the instances a workspace may now
    ...
    async def deletes(fresh):
        await fresh.delete_instance(_SCOPE, "Widget", "w1")
        assert await fresh.get_instance(_SCOPE, "Widget", "w1") is None
