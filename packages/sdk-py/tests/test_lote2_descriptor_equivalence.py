...
vivas (golden E descriptor gerados programaticamente DELAS — zero typos de
tradução); Phase B (este arquivo) é o teste de regressão pós-deleção.
...
CURATED_SUMMARY_DEFAULTS = {
    ...
    "Postmortem": {"title": "", "severity": "", "incident_at": "", "resolved_at": ""},
    "RiskRegister": {
...
def test_narrative_not_scope_inheritable(kernel):
    ...
    assert _port(kernel, "Narrative").scope_inheritable is False
...
def test_dep_filters_match_golden(kernel, kind):
    ...
    if kind in EMPTY_DEP_FILTER_KINDS:
...
def test_embed_fields_match_legacy_join(kernel, kind, fields):
    ...
    substituem as entradas no frozenset _EMBED_LEGACY (ratchet D4):
    field-join puro → texto byte-idêntico → zero re-embed."""
    assert _port(kernel, kind).embed_fields == fields
...
def test_narrative_stays_legacy_embed(kernel):
    """Narrative continua coberto pelo _EMBED_LEGACY: o branch legado junta
    spec.paragraphs com "\\n\\n" — não exprimível como field-join declarado
    (arrays são space-joined). Declarar embed aqui re-embeddaria toda
    Narrative com texto diferente."""
    assert _port(kernel, "Narrative").embed_fields is None
