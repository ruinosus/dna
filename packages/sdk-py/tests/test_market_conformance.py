...
Pattern per family: scan the native bundle → typed instance under the
...
def _emitted_files(kernel, scope: str, doc) -> dict[str, str]:
    payload = kernel.serialize_instance(scope, doc.kind, doc.name, doc.raw)
...
class TestRealSkills:
    ...
    def test_write_roundtrip_byte_identical(self, market):
        ...
        checked = fm_style = 0
        for s in [d for d in mi.instances if d.kind == "Skill"]:
    ...
    def test_fm_style_allowlist_is_confined_to_frontmatter_style(self, market):
        ...
        for name in sorted(SKILL_FM_STYLE_ALLOWLIST):
            doc = next((d for d in mi.instances if d.kind == "Skill" and d.name == name), None)
    ...
    def test_binary_assets_stay_on_disk(self, market):
        ...
        k, mi = market
        doc = next((d for d in mi.instances if d.kind == "Skill" and d.name == "canvas-design"), None)
...
class TestRealAgentsMd:
    def test_scan_typed_and_byte_roundtrip(self, fixture_scope):
        k, mi = fixture_scope
        doc = next((d for d in mi.instances if d.kind == "AgentDefinition" and d.name == "market-conformance"), None)
    ...
    def test_market_demo_agentsmd_roundtrip(self, market):
        k, mi = market
        doc = next((d for d in mi.instances if d.kind == "AgentDefinition" and d.name == "market-demo"), None)
...
class TestRealSouls:
    def test_starter_bundle_scan_typed(self, fixture_scope):
        ...
        _, mi = fixture_scope
        soul = next((d for d in mi.instances if d.kind == "Soul" and d.name == "starter"), None)
