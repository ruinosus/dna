...
class TestGuardrailIntegration:
    ...
    def test_safety_guardrail(self, mi):
        g = next((d for d in mi.documents if d.kind == "Guardrail" and d.name == "safety"), None)
    ...
    def test_code_quality_guardrail(self, mi):
        g = next((d for d in mi.documents if d.kind == "Guardrail" and d.name == "code-quality"), None)
    ...
    def test_pii_protection_guardrail(self, mi):
        g = next((d for d in mi.documents if d.kind == "Guardrail" and d.name == "pii-protection"), None)
    ...
    def test_review_ethics_guardrail(self, mi):
        g = next((d for d in mi.documents if d.kind == "Guardrail" and d.name == "review-ethics"), None)
    ...
    def test_reviewer_agent_references_guardrails(self, mi):
        agent = next((d for d in mi.documents if d.kind == "Agent" and d.name == "reviewer-agent"), None)
        assert "safety" in agent.spec.guardrails
