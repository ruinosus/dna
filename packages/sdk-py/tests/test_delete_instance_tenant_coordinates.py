...
An instance of a tenant-authored Kind wrote fine, appeared in ``list_instances``
and read back through ``get_instance`` — and ``delete_instance`` answered
``UnknownInstanceError: … not found in scope … — nothing to delete``. All four
...
The delete's existence check read WITHOUT a tenant::
    ...
    :604  read    get_instance(sc, port.kind, name, tenant=tenant)   ✓
    :758  delete  get_instance(sc, port.kind, name)                  ✗
...
concluded the instance did not exist. Two instances were unreachable in a live
...
must hold is that all four resolve the SAME instance for the SAME coordinates.
...
# whose instances live in a tenant layer.
_BUILTIN_KIND = "Agent"
...
async def _pg_source():
    ...
    ``instances`` primary key carries ``tenant`` (i-092), so base and overlay
...
async def kernel(request, tmp_path: Path):
    ...
    The Kind is seeded through ``kernel.write_instance`` rather than as a file so
    ...
    await k.write_instance(_SCOPE, "Genome", _SCOPE, _genome())
    await k.write_instance(_SCOPE, "KindDefinition", _ALIAS, _kinddef())
...
async def test_delete_finds_the_document_the_tenant_write_created(live):
    """RED before the fix, with the reported error verbatim:
        ...
        UnknownInstanceError: ProbeCheck 'c1' not found in scope 'probe-scope'
        — nothing to delete
    ...
    the instance was right there in every listing.
    """
...
async def test_the_quartet_agrees_on_one_document(live, kernel, kind, api_version):
    """Write, list, get and delete must resolve the SAME instance for the SAME
    ...
    """
    written = await D.write_instance_impl(
    ...
    listed = await D.list_instances_impl(
        live, kind=kind, scope=_SCOPE, tenant=_WORKSPACE, api_version=api_version)
    assert [d["name"] for d in listed["instances"]] == ["c1"]
    ...
    deleted = await D.delete_instance_impl(
    ...
    after = await D.list_instances_impl(
        live, kind=kind, scope=_SCOPE, tenant=_WORKSPACE, api_version=api_version)
    assert after["instances"] == []
    with pytest.raises(D.UnknownInstanceError):
...
async def test_the_existence_check_reads_at_the_delete_coordinates(live, kernel):
    ...
    real_get, real_delete = kernel.get_instance, kernel.delete_instance
    ...
    kernel.get_instance, kernel.delete_instance = spy_get, spy_delete
    try:
        await D.delete_instance_impl(
            live, kind=_TENANT_KIND, name="c1", api_version=_TENANT_AV,
            scope=_SCOPE, tenant=_WORKSPACE)
    finally:
        kernel.get_instance, kernel.delete_instance = real_get, real_delete
...
async def test_a_base_document_is_still_deletable_without_a_tenant(live):
    ...
    instance written with no tenant stays deletable with no tenant."""
    await D.write_instance_impl(
...
async def test_a_tenants_delete_does_not_reach_another_tenants_document(live):
    ...
    with pytest.raises(D.UnknownInstanceError):
        await D.delete_instance_impl(
            live, kind=_TENANT_KIND, name="c1", api_version=_TENANT_AV,
            scope=_SCOPE, tenant=_WORKSPACE)
    still_there = await D.get_instance_impl(
...
#
# ``write_instance_impl`` had the same family of asymmetry, latent: it checked
...
# decided it — see the comment at the check in ``instances.py``.
...
async def test_a_global_kind_resolves_the_same_document_at_both_coordinates(
    ...
    """The other half: for a GLOBAL Kind the two readings are the same instance.
    ``get_instance(tenant=X)`` probes the overlay and FALLS BACK to the base
    ...
    at_tenant = await kernel.get_instance(_SCOPE, _GLOBAL_KIND, "s-1", tenant=_WORKSPACE)
    at_base = await kernel.get_instance(_SCOPE, _GLOBAL_KIND, "s-1")
...
async def test_a_global_kind_checks_and_writes_at_the_same_coordinates(live, kernel):
    ...
    seen: dict[str, object] = {}
    real_get, real_write = kernel.get_instance, kernel.write_instance
    ...
    kernel.get_instance, kernel.write_instance = spy_get, spy_write
    try:
        await D.write_instance_impl(
            live, kind=_GLOBAL_KIND, name="s-1",
            spec={"title": "one", "description": "d", "status": "todo"},
        ...
        kernel.get_instance, kernel.write_instance = real_get, real_write
...
async def test_a_global_kind_still_merges_and_deletes(live):
    ...
    finds the stored instance and merges over it (rather than replacing it), and
    the delete still goes through."""
    await D.write_instance_impl(
        live, kind=_GLOBAL_KIND, name="s-1",
        spec={"title": "one", "description": "d", "status": "todo"},
        scope=_SCOPE, tenant=_WORKSPACE)
    second = await D.write_instance_impl(
    ...
    assert got["instance"]["spec"]["title"] == "one", (
        "the update dropped a field the caller did not send — the check no "
        "longer resolves the stored instance")
    assert got["instance"]["spec"]["status"] == "in-progress"
