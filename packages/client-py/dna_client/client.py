...
class DnaClient:
    ...
    def kind_graph(
        ...
        **Rank ``unresolved`` by ``origin``, never by ``reason``.**
        ``declared`` and ``composition`` rows are declarations the model cannot
        honour — an authoring error. ``shape-inferred`` rows are the projection
        guessing from a field NAME and are usually not references at all (an
        OAuth ``client_id``, a Stripe customer id).
