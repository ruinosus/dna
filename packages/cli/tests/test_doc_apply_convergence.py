...
The fix compares raw-against-raw: `write_instance` persists the RAW doc, so
...
from dna_cli.instance_cmd import _apply_one
...
def test_reapply_of_identical_file_is_unchanged_for_a_kind_with_defaults(
    ...
    assert kernel.write_instance.await_count == 0, (
...
def test_genuinely_changed_file_is_still_updated(monkeypatch, capsys):
    ...
    UPDATED and reach write_instance — convergence must not over-rotate into
    never writing."""
    monkeypatch.setattr(instance_cmd, "_tenant_write_note", lambda t: (None, None))
    s, kernel = _mock_session(_stored_document(_AGENT_RAW))
    ...
    written = kernel.write_instance.await_args.args[3]
