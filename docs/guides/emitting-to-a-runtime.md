...
runtime — **`agno`** (the default when `--target` is omitted) or
`agent-framework`; each emits its own `agent` + `serving` pair from the *same*
Copilot source:

```console
$ dna emit memory-copilot --target agent-framework --out app/   # same source, MS-AF app
$ dna emit memory-copilot --out app/                             # no --target → agno
```

> **LangGraph copilots are not generated by `dna emit`.** The `langgraph`
> target's servable `copilot` case is retired — a hand-rolled `StateGraph`
> scaffold is the wrong shape for a fast-moving host app. Build a servable
> LangGraph copilot programmatically instead, via
> `dna.runtime.build_copilot(...)` (the SDK `runtime` extra), which applies the
> same DNA middlewares (allowlist, compose-prompt, HITL) without a code-gen
> step. `dna emit <agent> --target langgraph` on a plain **Agent** (not a
> Copilot) is unaffected — it still emits a single `create_react_agent` module.
