...
| `created_at` |  | When the document was created (ISO-8601). VOLATILE means excluded from canonical_digest, NOT supplied by the runtime — nothing in the write path stamps it, so a document written without it comes back with no created_at at all. The Kind-authoring door therefore stamps it deliberately, and the audit view projects it. It means the BIRTH of the document and is PRESERVED across edits — the authoring door rebuilds the spec from scratch and persists it, so it carries the existing value forward and stamps `now` only when the document does not yet exist; 'when the document was created' has to stay true after the first edit. For when the pending shape was proposed, read proposed_at. A write-time stamp, if one ever lands, wins. |
...
| `revoked_by` |  | Who REVOKED this Kind — the THIRD state, and deliberately not the absence of the second. Clearing approved_by is indistinguishable from never having approved, and never-approved means UNREGISTERED, which means documents are accepted with no validation at all — so a revocation implemented as a plain un-approval loosens instead of tightening. The revoked fact is therefore persisted here; the Kind stays REGISTERED (being known is what stops revocation meaning 'accepts anything'); new documents of it are REFUSED; and existing ones read back MARKED invalid — never deleted, never an error, because the data did nothing wrong and the audit needs to see what existed. Reversible: approving again clears this pair and validity returns, because validity follows the Kind's CURRENT state and is never a stamp on the document. approved_by is deliberately left standing beside it — the audit must keep who conferred effect in the first place. |
...
| [AgentCatalogEntry](record.md#agentcatalogentry) | `a2a-agent-catalog-entry` | `github.com/ruinosus/dna/a2a/v1` |
...
| [App](record.md#app) | `helix-app` | `github.com/ruinosus/dna/v1` |
...
| [KindNamespace](record.md#kindnamespace) | `tenant-kind-namespace` | `github.com/ruinosus/dna/tenant/v1` |
...
| [RemoteAgent](record.md#remoteagent) | `a2a-remote-agent` | `github.com/ruinosus/dna/a2a/v1` |
...
| [Sprint](record.md#sprint) | `sdlc-sprint` | `github.com/ruinosus/dna/sdlc/v1` |
