...
| `memory.policies[].applies_to.memory_type` | string |  | Um de: `episodic`, `semantic`, `procedural`. |
...
| Field | Type | Required | Description |
| --- | --- | --- | --- |
| `affect` | string | yes | Emotional tone. Evocative palette (Spec §15 decision): triumph, regret, surprise, wistful, ominous. Um de: `triumph`, `regret`, `surprise`, `wistful`, `ominous`. |
...
| `memory_type` | string |  | CoALA taxonomy — ORTHOGONAL to the Semon EngramState (type vs state): episodic = what happened (instance/sequence); semantic = a generalized fact about the world/user; procedural = how to act (a skill/rule). Absent = untyped (legacy). Um de: `episodic`, `semantic`, `procedural`. |
