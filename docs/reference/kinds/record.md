...
| `cost_per_1m_input_usd` | number \| null |  | USD per 1M input tokens. NOT informational any more — a cost reader turns it into the money the product shows. Null means NOT QUOTED, never zero; a null read as 0.0 would report the most expensive model as free. Half a quote (this field without cost_per_1m_output_usd) is not half a price — the whole cost comes back NOT CALCULABLE, never a partial sum. |
| `cost_per_1m_output_usd` | number \| null |  | USD per 1M output tokens. Same contract as cost_per_1m_input_usd — null means NOT QUOTED, and the currency is in the field name (_usd), never presumed. |
| `cost_quoted_at` | string \| null |  | ISO-8601 date/datetime this price was QUOTED. i-101, decided 08/08/2026. While the cost fields were informational a stale price hurt nobody; now that a reader turns them into the money the product shows, a stale accountable number produces a wrong account wearing the face of a measurement. Past the reader's age ceiling the reading marks the money SUSPECT (the is_suspect qualifier, printed on the same line as the number, in the mould of the FLOOR marker). NULL or absent is NOT read as recent — it is UNKNOWN AGE, and unknown age is suspect too, because presuming the good side is exactly how the old cap lived in nobody's code. OPTIONAL on purpose — see the note on `required` above the schema. |
