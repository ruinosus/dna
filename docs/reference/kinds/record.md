...
An App is the NAMED composition of copilots (record plane) — the installable/sellable unit of the spec-app-como-composicao decisions (2026-08-05). A kit is the PACKAGE (how a flow ships); an App is the INSTANCE (how it is used, navigated and charged). It groups existing Copilot docs under one identity (title/description/icon), carries the plan entitlement (``requires_plan`` — enforced by the serving runtime, never by the kernel) and the console renders ``/app/<name>`` from this instance alone. ``copilots`` is a declared RELATION to ``Copilot`` — the write validates existence and pickers come for free (i-040). It is OPTIONAL and carries no ``minItems``: an App with no copilot is a VALID App — a service that serves and has not been given one yet, or one that never will (dna-cloud's ``worker`` scales on KEDA and serves no copilot at all). Requiring one was the mark of the older design, where the App was a shopfront OVER copilots; it is the container now, and a container exists before what lives inside it. Since ``spec-app-e-o-servico`` (2026-08-07) an App is also THE SERVICE that runs it — one deployment, identified by ``metadata.name``. One of those fields describes the CODE and is shared by sibling Apps (``service_name``, the ``apps/<name>/`` directory — the same axis ``Solution.services[].name`` addresses); the others describe the DEPLOYMENT and are per App (``port``, the in-container ``targetPort``, ``ingress`` and ``can_sleep``). Code to deployment is 1:N, measured: dna-cloud runs 9 deployable services over 4 ``apps/`` directories, and ``apps/mcp/`` alone serves ``mcp``/``mcp-entra``/``mcp-ws`` — same image, same port, different identity authority. All optional, because the 2 live Apps predate them. ``can_sleep`` is the one that pays for the change: a container app that cannot sleep carries a fixed replica at ~US$ 90/month forever (measured — the dna-cloud ``copilot`` service was US$ 94,43 of a US$ 230,29 bill), and until now no declaration anywhere answered that question. Absent is never a presumed ``true``: it means the cost question was never asked, which is a finding to report. ``spec-campo-opcional-por-evidencia`` (2026-08-07) added the RULER that decides which Kind a stack field belongs to, and it is mechanical: the App carries what the WIRING needs; ``Solution.services[].answers`` carries what the TEMPLATE needs. Measured against the three wiring fragments of ``templates/app-container``, ``python_module`` appears in ZERO of them (and in 4 code files), so it was REMOVED from this Kind and lives in ``answers`` with the rest of the template vocabulary — which is what makes dna-cloud's Next.js ``portal`` fit with no exception at all, and what lets a Node template ask ``package_name`` without this Kind changing a line. By the same measurement ``ingress`` (1 wiring use) belongs here, and it is what makes an absent ``port`` an ANSWER rather than a gap: dna-cloud's ``worker`` is not "an App whose port does not apply", it is an App that does not serve — ``ingress: none``, from which no port follows by definition. Declaring both is a contradiction and is refused at write time. A generic "not applicable" mechanism was built first and then removed: once ``python_module`` left, exactly ONE case remained across nine services, and one case does not pay for a mechanism. The same spec also made a field loosen only when REALITY presents a legitimate case, never for convenience, with the evidence written in that field's ``description``.
...
| `copilots` | array |  | Os copilotos que este App serve, por nome de instância — cada um traz seus fluxos (surfaces) para debaixo desta identidade. A referência é validada na gravação (`spec.relations`). ⚠️ OPCIONAL, e sem `minItems`: uma lista vazia é um estado LEGÍTIMO, não um erro. Um App sem copiloto é um App válido — é um serviço que atende e ainda não recebeu nenhum, ou que nunca vai receber (o `worker` do dna-cloud escala por KEDA e não serve copiloto nenhum). Exigir pelo menos um era a marca do desenho ANTERIOR, em que o App era vitrine SOBRE copilotos; agora ele é o contêiner, e o contêiner existe antes do que vive dentro dele. |
...
| `ingress` | string |  | Quem alcança este App. `external` = de fora do ambiente; `internal` = só de dentro dele; `none` = **não atende ninguém**. Fato sobre o DEPLOYMENT (1 uso na fiação, 0 no código gerado), e por isso mora aqui e não no `answers`. ⭐ `none` é o que torna a porta ausente uma RESPOSTA e não uma lacuna, e é a evidência do `worker` do dna-cloud: ele escala por KEDA sobre a fila e não serve ninguém. O fato é que ele NÃO ATENDE; a porta ausente é CONSEQUÊNCIA disso, por definição, já que `port` É o `targetPort` do ingress. Declarar aqui é uma resposta de humano — não a inferência pelo formato de que esta decisão está saindo. ⛔ `ingress: none` COM um `port` é contradição, e é recusado na gravação (`allOf` abaixo). ✅ O template fala os três desde 08/08/2026 (i-099): `copier.yml` oferece `none`, e lá o `port` é uma pergunta gated — um `worker` é gerável, e o que sai não carrega porta em lugar nenhum. Ausente significa que ninguém disse quem alcança este App; é um fato a reportar, nunca um `internal` presumido. Um de: `external`, `internal`, `none`. |
| `nav_order` | integer |  | Ordem relativa na navegação do console (menor primeiro). Ausente = ordem alfabética. |
| `port` | integer |  | A porta que o processo escuta DENTRO do contêiner — o `targetPort` do Container App, o `container_port` do template. ⚠️ NÃO é a porta publicada no host. Medido no dna-cloud em 07/08/2026 (`infra/bicep/containerapps.bicep`): `mcp`, `rest`, `copilot` e `a2a` escutam todos em 8080, e só o portal difere (3000, Next.js). As portas 8100 / 8102 / 8090 / 8091 / 8182 que aparecem no `docker-compose.dev.yml` são MAPEAMENTO DE HOST para o dev local, e declarar uma delas aqui geraria um `targetPort` apontando para onde nada escuta. O que distingue `mcp` de `mcp-entra` não é a porta — é o ENV (a autoridade de identidade), com a mesma imagem e a mesma porta; é mais uma razão para a identidade do App ser o `metadata.name`, já que `service_name` e `port` colidem entre Apps irmãos por desenho. Declarada e não presumida porque o default de um framework não é um compromisso — o `dna.runtime` serve numa porta, o `dna api serve` em outra, e quem lê a frota precisa saber qual sem abrir o Dockerfile. ⭐ EVIDÊNCIA para ser OPCIONAL (`spec-campo-opcional-por-evidencia`, 07/08/2026): o `worker` do dna-cloud NÃO ATENDE, de propósito — ele escala por KEDA sobre a fila. "Uma porta que ninguém chama é só uma porta aberta": declarar uma aqui não descreveria o worker, abriria superfície que ele foi desenhado para não ter. Medido no dogfood dos nove serviços reais (`ruinosus/dna-cloud#370`). ⚠️ E a ausência SOZINHA não basta, porque diz duas coisas opostas. Quem a desambigua é `ingress`, e não um mecanismo genérico: o `worker` declara `ingress: none`, e aí não ter porta é RESPOSTA — a contradição de declarar as duas coisas é recusada na gravação. Um App com `ingress: external` e sem `port` é o outro caso, e esse segue cobrado pelo relatório, toda execução. |
...
A Copilot is a declarative, servable AG-UI copilot backend — a binder that composes one-or-more mounted Agents (each with its own Tools and optional MCPFederation) into a single servable ``/agui`` app. It carries only the copilot-level concerns that don't belong on any single Agent ``mounts`` (where agents serve), ``serving`` (the transport), ``tenant`` (inbound-tenant propagation), ``hitl`` (the approval card for gated write tools), ``knowledge`` (RAG collections + the vector store it may read), ``persistence`` (checkpoint/memory/cache/conversation storage backends), ``hosting`` (self-hosted vs a managed runtime), and ``frontend`` (console hints). Instructions and persona stay on the mounted Agent — a Copilot never re-declares them. One instance emits a servable backend (Agno today), the single evolution point DNA Cloud's copilots consume. Stored as ``copilots/<name>.yaml`` — marketplace-shareable as a bundle. Since ``spec-app-e-o-servico`` (2026-08-07) it also declares where it RUNS and what it is WORTH — ``runs_in`` is an OPTIONAL relation to the ``App`` whose service process serves it (execution, not composition — hence no ``inverse_of``; see the descriptor header), and ``value_per_outcome`` ({human_minutes, hourly_cost, currency}) is the declared worth of one successful outcome. That last one is the single number in the chain that cannot be measured — the manual path is timed by no telemetry — so it must come from someone who knows the business. It is the counterpart of ``App.can_sleep`` — the App declares the cost, the Copilot the yield. Since ``spec-a-fabrica`` (2026-08-07) it also declares WHERE IT CAME FROM — ``created_by`` is an OPTIONAL, reflexive relation to the ``Copilot`` that produced it — the factory is made of its own material, and a resolved by-name relation means chain depth is answered by ``dna graph refs`` with no new mechanism. Absent is UNANSWERED and never "hand-written"; ``dna copilot provenance`` reports the silent ones apart from the dangling ones.

**Spec fields**

| Field | Type | Required | Description |
| --- | --- | --- | --- |
| `created_by` | string |  | The Copilot that PRODUCED this one, by instance name — provenance of the agent, the counterpart of `Solution.services[].template.ref` for generated code. Reflexive on purpose: a copilot is made by a copilot (the `copiloto-criador` composes `CopilotBlueprint` instances and "saving creates the copilot"), which is the reflexive loop `spec-a-fabrica` measured. Resolved and enforced by name, so it draws a real edge — and CHAIN DEPTH ("a copilot that made another that made another") needs no new mechanism: `dna graph refs Copilot <name> --direction out --depth N` walks it, and `--direction in` answers "what did this one make?". ⚠️ OPTIONAL, and absent is UNANSWERED — never "written by hand". The 7 live copilots measured on 2026-08-07 include at least one that WAS generated and has no way to say so, because the field did not exist when it was born; reading their silence as an answer would fabricate a past. `dna copilot provenance` reports the unanswered ones and, in a separate bucket, the DANGLING ones (a creator that no longer exists), because those two have different owners. This does NOT point at the declaration that produced it: the ANSWERS live in the `CopilotBlueprint` record (a DNA Cloud Kind, persisted since i-137 — and measured to survive an answer that the materialization erases behind a condition, which is exactly why it has to exist). See the descriptor header for why naming that Kind here was refused. |
...
| `revivals` | array |  | APPEND-ONLY log of the periods this memory spent retired and came back from — one entry per completed `forget` → `revive` cycle (i-139). Written only by `revive`; never edited, never reordered. `forget` promised "revivable" in its own docstring from the day it existed, and nothing revived: a grep for `unforget\|restore\|revive\|undelete\|recover` across the SDK returned zero. This field is the half that was missing, and it is what lets the way back be AUDITABLE rather than an erasure — the date the memory stopped being in force, and whatever had superseded it, survive the revival that ended them. Named after the EVENT, not the state: an entry exists only because a revive happened. A retirement still IN FORCE is not here — it is the live `valid_to`. So an empty list means "never came back", which is true of a memory retired right now, and no entry is hiding elsewhere. ⚠️ THE LIMITATION THIS FIELD ACCEPTS, and it is a founder decision (i-139, 07/08/2026), not an oversight. `dna_instances` holds ONE row per instance (PRIMARY KEY scope+kind+api_version+name+tenant), so the `valid_at` column knows only the CURRENT window — even though its `EXCLUDE USING gist (id WITH =, valid_at WITH &&)` would happily hold many disjoint ones. The constraint permits the second row; the primary key forbids it. Therefore `recall(as_of=T)` for a T inside a past gap answers from the TRANSACTION axis (what the store BELIEVED then), not the world-time axis (whether the fact WAS true then). The gaps are readable here, exactly and in the present; what is not available is a world-time QUERY across them. Buying that back means a table rewrite plus a new cardinality for every read and write, measured against 14 memories (2 retired, 0 superseded) and declined. |
| `revivals[].revived_at` | string |  | When it came back. The end of the gap. |
| `revivals[].revived_by` | string |  | WHO revived it, resolved by the FACE from a verified request — never from a caller-supplied body, because attribution a caller can forge is not attribution. Absent when the caller declared nothing: the key is omitted rather than null, so "not recorded" and "recorded as nothing" stay distinguishable. |
| `revivals[].superseded_by` | string |  | The memory that had superseded this one during the gap, if any. Absent when the retirement named no successor. |
| `revivals[].valid_to` | string |  | The `valid_to` this retirement carried, VERBATIM — when the memory stopped being in force. The start of the gap. |
...
| `superseded_by_memory` | string |  | Name of the memory that invalidated this one (not `superseded_by` — that's an ADR dep_filter token). Pairs with valid_to for point-in-time audit, and goes where valid_to goes — `revive` moves both into the `revivals` entry, because a memory back in force is not superseded. |
...
| `valid_to` | string |  | World-time validity end. Set when superseded/contradicted — the memory is INVALIDATED, never hard-deleted. Default recall excludes valid_to<now. Removed by `revive`, which files the closed interval into `revivals` first — so this is the CURRENT retirement only, never the history of them. |
...
| `cost_per_1m_input_usd` | number \| null |  | USD per 1M input tokens. NOT informational any more — dna.runtime.roi reads it (price_book / gather_prices) and it feeds the money the product shows. Null means NOT QUOTED, never zero; a null read as 0.0 would report the most expensive model as free. Half a quote (this field without cost_per_1m_output_usd) is not half a price — the whole cost comes back NOT CALCULABLE, never a partial sum. |
| `cost_per_1m_output_usd` | number \| null |  | USD per 1M output tokens. Same contract as cost_per_1m_input_usd — read by dna.runtime.roi, null means NOT QUOTED, and the currency is in the field name (_usd), never presumed. |
| `cost_quoted_at` | string \| null |  | ISO-8601 date/datetime this price was QUOTED. i-101, decided 08/08/2026. While the cost fields were informational a stale price hurt nobody; now that dna.runtime.roi turns them into the money the product shows, a stale accountable number produces a wrong account wearing the face of a measurement. Past dna.runtime.roi's age ceiling the reading marks the money SUSPECT (the is_suspect qualifier, printed on the same line as the number, in the mould of the FLOOR marker). NULL or absent is NOT read as recent — it is UNKNOWN AGE, and unknown age is suspect too, because presuming the good side is exactly how the old cap lived in nobody's code. OPTIONAL on purpose — see the note on `required` above the schema. |
| `cost_source` | string \| null |  | WHERE the quote came from — the provider's pricing page, a negotiated contract, an estimate. i-101. It changes what the number means (a contract price is stable for a year; a pricing page is not) and it travels into the cost's `source` line so whoever reads the money reads its provenance. OPTIONAL, like cost_quoted_at. |
...
A Solution is the DECLARATION of a repo generated from Copier templates, plus the ANSWERS that generated it — never the code and never the template body. It is the second view of the `.copier-answers.<service>.yml` files a `dna solution new` writes, and the only one that OUTLIVES them: a `when:`-gated answer is erased from the file when its condition stops holding, and a record that survives the file is the only place such a value can survive (measured — see `docs/guides/solution-scaffolding.md`). `dna solution update --solution <name>` reads the answers back from here and re-passes them to Copier, which is what makes a version floor recorded here reach the generated tree instead of staying behind in silence. One entry in `services[]` per answers file — one template overlaid N times is the design, not one template of the whole repo. The per-service `answers` map is deliberately free-form — the vocabulary belongs to the TEMPLATE, and nothing is promoted out of it. Since `spec-app-e-o-servico` (2026-08-07) this record holds the PROVENANCE OF THE RENDER and nothing else. `pode_dormir` was the one fact it used to promote, and it moved to `App.can_sleep`, because `services[]` is one entry per DEPLOYMENT and an `App` now IS the deployment — the same granularity, and the bill is per deployment. The Copier answer itself stays here verbatim inside `answers`; what ended was its promotion to a field of its own. `services[].name` is the azd service name, which is string-for-string the `App`'s `metadata.name` — the key between the two lists. That relation cannot be declared from inside `items` (the kernel names a relation by a FIRST-LEVEL spec field), so the declared, enforced relation to `App` is `apps` — which, with App = deployment, denotes the same set. Since `spec-campo-opcional-por-evidencia` (2026-08-07) `services` is OPTIONAL, and the evidence is dna-cloud itself: it was never generated from a template, so there is no `.copier-answers` anywhere, and a `required` turned "this repo has no render provenance" into "a field is missing" — losing the reason. Absent is an ASSERTION (no provenance); `minItems: 1` stays so that `services: []`, which asserts neither, is still refused. ⚠️ Consumers must NOT read "no services" as "nothing to check": `unanswered_cost_question` reads `apps[]` — the enforced relation, which is the set of deployments by definition — precisely so a Solution without `services` cannot answer "all answered" by vacuity.

**Spec fields**

| Field | Type | Required | Description |
| --- | --- | --- | --- |
| `apps` | array |  | Os `App`s que esta solução ENTREGA, por nome de instância. Referência validada na gravação (`spec.relations`), e — desde `spec-app-e-o-servico` — a ÚNICA relação declarada que liga este registro ao `App`. Como o `App` agora É o serviço, esta lista e o conjunto de `services[].name` denotam as MESMAS coisas: nove serviços gerados são nove `App`s. Vazio significa que os `App`s ainda não foram declarados, não que não existam — e a nota antiga "o dna-cloud gera nove serviços e nenhum App" descrevia o modelo anterior, em que `App` era só o agrupamento vendável do console. |
...
| `services` | array |  | Uma entrada POR CAMADA — e uma camada é um answers file, que é um app. Nove serviços sobre quatro imagens são nove entradas aqui, quatro `template.src` distintos, e nove updates independentes. Nunca um template do repo inteiro (§4.4 regra 3). ⭐ EVIDÊNCIA para ser OPCIONAL (`spec-campo-opcional-por-evidencia`, 07/08/2026): o **dna-cloud nunca foi gerado por template**. Não há `.copier-answers` porque não houve render — a primeira solução real desta casa tem ZERO camadas, e não por descuido. Enquanto isto era `required`, o dogfood dos nove serviços reais recusava a `Solution` inteira (medido em `ruinosus/dna-cloud#370`: os nove Apps passavam, e só ela ficava), e a recusa não dizia "este repo não veio de template" — dizia que faltava um campo. O motivo sumia, que é o pior da recusa. Ausente é uma AFIRMAÇÃO: esta solução não tem procedência de render. É legível, e é diferente de uma solução gerada cujas camadas ninguém registrou — essa tem `services[]` e o `update` a alcança. ⚠️ `minItems: 1` FICA, e a combinação é deliberada: opcional dá conta do repo que nunca foi renderizado; `minItems` recusa o `services: []`, que não seria nem uma coisa nem outra. O schema impede besteira; a completude é um relatório. ⚠️⚠️ E quem consome isto NÃO pode mais ler "sem `services` = nada a conferir". `unanswered_cost_question` fazia `if not names: return []` sobre esta lista, e com o campo opcional uma `Solution` sem ele responderia "tudo respondido" — verde por vacuidade, a classe de defeito que já cegou três guardas nesta casa. A fonte do relatório passa a ser `apps[]`, a relação imposta, que é o conjunto de deployments por definição e existe em repo gerado ou não (`Story/s-relatorio-do-que-falta`). |
| `services[].answers` | object |  | As respostas desta camada, VERBATIM, no vocabulário do template. ⭐ É a casa do que o TEMPLATE precisa, pela régua de `spec-campo-opcional-por-evidencia` — um campo que não aparece em NENHUM dos três fragmentos de fiação não descreve o deployment e mora aqui, não no `App`. Foi por essa medição que `python_module` desceu do `App` para cá em 07/08/2026 (0 usos na fiação, 4 no código gerado) — e é o que faz um `portal` Next.js caber sem exceção, e um template Node trazer `package_name` sem que o `App` mude. Mapa livre de propósito - o conjunto de perguntas é do template e muda com ele, e nenhuma chave é obrigatória porque uma resposta atrás de `when:` legitimamente desaparece (medido - exigir uma seria recusar a gravação exatamente do caso que este registro existe para socorrer). Copier já filtra o próprio bookkeeping (`_src_path`, `_commit`) para os campos `template` acima; ele não se repete aqui. |
...
| `services[].name` | string | yes | O nome do serviço — o que `dna solution update --service <name>` recebe, e o que o azd chama de serviço. ⭐ É, string por string, o `metadata.name` do `App` correspondente: uma entrada aqui é um DEPLOYMENT, e desde `spec-app-e-o-servico` o `App` É o deployment. Nove serviços gerados são nove entradas aqui e nove `App`s, e a chave entre as duas listas é este campo. ⚠️ A relação NÃO é declarável a partir daqui — o kernel nomeia uma relação por um campo de PRIMEIRO NÍVEL do spec (`kind_graph`, código `top_level_properties_only`), e um ponteiro dentro de `items` é invisível para ele. Quem carrega a relação imposta com o `App` é `apps[]`, no primeiro nível. |
