...
| `engine` | string |  | Whiteboard renderer (per-canvas, not convertible). 3 engines kept after exploration: - tldraw — rich UI, multi-page (commercial license for prod) - excalidraw — MIT, hand-drawn casual sketch - drawio — Apache 2.0, formal BPMN / architecture diagrams Um de: `tldraw`, `excalidraw`, `drawio`. |
| `excalidraw_store` | object |  | Excalidraw scene (elements+appState). engine=excalidraw. |
| `last_drawn_by` | string |  | Who touched the canvas most recently. Um de: `user`, `jarvis`, `mixed`. |
...
| --- | --- | --- | --- |
| `rules` | array |  |  |
| `scope` | string |  | Um de: `input`, `output`, `both`. |
| `severity` | string |  | Um de: `warn`, `error`, `hard`. |
...
| `reinforcement` | string |  | How Lumi reacts to correct answers. 'celebrate' = set_pose celebrating + warm phrase. Um de: `celebrate`, `gentle`, `neutral`. |
| `skill` | string |  | Um de: `reconhecer`, `identificar`, `parear`, `repetir`, `associar`, `contar`, `ordenar`. |
...
| `steps[].expected_concept` | string |  | Optional Pictogram concept slug to show via show_pictogram(). Required on `test` steps so record_attempt knows the target. |
| `steps[].hint_ladder` | array |  | Optional 1-3 progressively stronger hints if Mateus errs (silhouette → color → sound). |
| `steps[].kind` | string | yes | present = introduce concept; repeat = ask Mateus to repeat/imitate; test = ask Mateus to identify (calls record_attempt); celebrate = positive reinforcement; review = recap before next step. Um de: `present`, `repeat`, `test`, `celebrate`, `review`. |
| `steps[].on_correct` | string |  | Lumi's reaction on success. Default: 'Isso!' for test, advance otherwise. |
| `steps[].on_incorrect` | string |  | Lumi's reaction on failure. Default: gentle re-prompt + show pictogram again. |
| `steps[].prompt` | string | yes | Short PT-BR phrase Lumi speaks at this step. ≤6 words ideal. |
...
| `auth.env` | string |  | Name of the env var holding the secret value. |
| `auth.header` | string |  | Header to carry the value (header_env only; bearer_env implies Authorization: Bearer). |
| `auth.kind` | string |  | Um de: `none`, `bearer_env`, `header_env`. |
...
| `health_check` | object |  |  |
| `health_check.interval_s` | integer |  |  |
| `health_check.timeout_s` | integer |  |  |
...
| `transport` | string |  | How to reach the server: stdio subprocess (default, v1) or Streamable HTTP. Um de: `stdio`, `streamable_http`. |
...
| `findings[].evidence_rating` | string | yes | Um de: `evidence-based`, `opinion-practice`, `anecdotal`. |
| `findings[].id` | string | yes | Stable id within this Research (e.g. 'f-diataxis-adoption'). |
| `findings[].source_refs` | array |  | Reference names supporting this finding. |
...
| `methodology` | string | yes | Um de: `web-search-curated`, `literature-review`, `interview`, `field-study`, `experiment`, `synthesis`, `other`. |
...
| `overall_confidence` | string |  | GRADE-inspired confidence rating. Computable: high if >=80% findings evidence-based, moderate 60-80, low 40-60, very-low <40. Author can override. Um de: `high`, `moderate`, `low`, `very-low`. |
...
| `recommendations[].backed_by_findings` | array |  | Finding ids supporting this recommendation. |
| `recommendations[].clinical_decision` | boolean |  |  |
| `recommendations[].depends_on` | array |  |  |
| `recommendations[].effort_hours` | number |  |  |
| `recommendations[].id` | string | yes |  |
| `recommendations[].priority` | string | yes | Um de: `high`, `medium`, `low`. |
| `recommendations[].status` | string |  | Um de: `proposed`, `accepted`, `rejected`, `implemented`, `blocked`. |
| `recommendations[].summary` | string | yes |  |
...
| `updated_at` | string |  |  |
| `visibility` | string |  | scope-private = only this scope sees it. shared = discoverable across scopes. Um de: `scope-private`, `shared`. |
...
| `config_scope` | string |  | Domain category (drives sidebar grouping + .claude/settings.json key). Um de: `api`, `auth`, `environment`, `git`, `global`, `hooks`, `model`. |
| `created_at` | string |  |  |
...
| `role` | string | yes | Per-tenant role. `owner` is the user who provisioned the tenant (set by POST /tenants). `admin` can manage members + tenant settings. `member` can read/write scope docs. `viewer` is read-only. Um de: `owner`, `admin`, `member`, `viewer`. |
| `status` | string |  | `pending` for invites awaiting first login; `active` after first login (route handler transitions); `revoked` after admin removes. Um de: `active`, `pending`, `revoked`. |
...
| `kind_of_test` | string | yes | Um de: `manual`, `smoke`, `e2e`, `regression`, `integration`. |
| `labels` | array |  |  |
...
| `status` | string |  | Um de: `draft`, `active`, `deprecated`. |
| `steps` | array | yes |  |
| `steps[].action` | string | yes | What the tester does. |
| `steps[].expected` | string | yes | Observable expected result. |
| `steps[].where` | string |  | Where in the product to do it (route/screen) so a non-dev can follow, e.g. '/scopes/:scope/sdlc/v2?t=focus'. |
| `updated_at` | string |  |  |
...
| `labels` | array |  |  |
| `notes` | string |  |  |
| `outcome` | string | yes | Um de: `pass`, `fail`, `partial`, `blocked`. |
| `screenshots` | array |  | Run-level evidence prints, Asset-backed (asset name + blob path), NOT inline base64. |
| `screenshots[].asset` | string | yes |  |
| `screenshots[].blob` | string |  |  |
| `screenshots[].mime` | string |  |  |
| `step_results` | array |  |  |
| `step_results[].notes` | string |  |  |
| `step_results[].result` | string | yes | Um de: `pass`, `fail`, `skip`. |
| `step_results[].screenshot` | string |  | Screenshot evidence for this step (data URL or asset ref). |
| `step_results[].step_index` | integer | yes |  |
...
| `owner` | string |  |  |
| `palette` | object | yes |  |
| `palette.accent` | object |  | Defaults to primary if omitted. |
| `palette.accent.dark` | object | yes |  |
| `palette.accent.dark.h` | integer | yes | Hue 0-360 |
| `palette.accent.dark.l` | integer | yes | Lightness 0-100% |
| `palette.accent.dark.s` | integer | yes | Saturation 0-100% |
| `palette.accent.light` | object | yes |  |
| `palette.accent.light.h` | integer | yes | Hue 0-360 |
| `palette.accent.light.l` | integer | yes | Lightness 0-100% |
| `palette.accent.light.s` | integer | yes | Saturation 0-100% |
| `palette.destructive` | object |  | Same color, two lightness/saturation tweaks for light vs dark mode. |
| `palette.destructive.dark` | object | yes |  |
| `palette.destructive.dark.h` | integer | yes | Hue 0-360 |
| `palette.destructive.dark.l` | integer | yes | Lightness 0-100% |
| `palette.destructive.dark.s` | integer | yes | Saturation 0-100% |
| `palette.destructive.light` | object | yes |  |
| `palette.destructive.light.h` | integer | yes | Hue 0-360 |
| `palette.destructive.light.l` | integer | yes | Lightness 0-100% |
| `palette.destructive.light.s` | integer | yes | Saturation 0-100% |
| `palette.info` | object |  | Same color, two lightness/saturation tweaks for light vs dark mode. |
| `palette.info.dark` | object | yes |  |
| `palette.info.dark.h` | integer | yes | Hue 0-360 |
| `palette.info.dark.l` | integer | yes | Lightness 0-100% |
| `palette.info.dark.s` | integer | yes | Saturation 0-100% |
| `palette.info.light` | object | yes |  |
| `palette.info.light.h` | integer | yes | Hue 0-360 |
| `palette.info.light.l` | integer | yes | Lightness 0-100% |
| `palette.info.light.s` | integer | yes | Saturation 0-100% |
| `palette.primary` | object | yes | Same color, two lightness/saturation tweaks for light vs dark mode. |
| `palette.primary.dark` | object | yes |  |
| `palette.primary.dark.h` | integer | yes | Hue 0-360 |
| `palette.primary.dark.l` | integer | yes | Lightness 0-100% |
| `palette.primary.dark.s` | integer | yes | Saturation 0-100% |
| `palette.primary.light` | object | yes |  |
| `palette.primary.light.h` | integer | yes | Hue 0-360 |
| `palette.primary.light.l` | integer | yes | Lightness 0-100% |
| `palette.primary.light.s` | integer | yes | Saturation 0-100% |
| `palette.success` | object |  | Defaults to primary if omitted. |
| `palette.success.dark` | object | yes |  |
| `palette.success.dark.h` | integer | yes | Hue 0-360 |
| `palette.success.dark.l` | integer | yes | Lightness 0-100% |
| `palette.success.dark.s` | integer | yes | Saturation 0-100% |
| `palette.success.light` | object | yes |  |
| `palette.success.light.h` | integer | yes | Hue 0-360 |
| `palette.success.light.l` | integer | yes | Lightness 0-100% |
| `palette.success.light.s` | integer | yes | Saturation 0-100% |
| `palette.warning` | object |  | Same color, two lightness/saturation tweaks for light vs dark mode. |
| `palette.warning.dark` | object | yes |  |
| `palette.warning.dark.h` | integer | yes | Hue 0-360 |
| `palette.warning.dark.l` | integer | yes | Lightness 0-100% |
| `palette.warning.dark.s` | integer | yes | Saturation 0-100% |
| `palette.warning.light` | object | yes |  |
| `palette.warning.light.h` | integer | yes | Hue 0-360 |
| `palette.warning.light.l` | integer | yes | Lightness 0-100% |
| `palette.warning.light.s` | integer | yes | Saturation 0-100% |
...
| `tags` | array |  |  |
| `updated_at` | string |  |  |
| `vibe` | string |  | Visual vibe tag for grouping. Um de: `neutral`, `playful`, `professional`, `warm`, `cool`, `minimal`, `vibrant`, `high-contrast`. |
...
| `consent` | object |  |  |
| `consent.memory_persistence` | boolean |  | False = agents DO NOT write Remembrances/Engram stamped with this user_id. Existing memories stay. |
| `consent.profile_used_in_prompts` | boolean | yes | Master gate. False = profile exists but agents treat user as anonymous. True = injected into system prompt. |
| `consent.voice_recording_consent` | boolean |  | Per local law (GDPR / LGPD), explicit opt-in for VoiceEpisode transcript persistence. |
...
| `languages` | object |  |  |
| `languages.accepted` | array |  | Languages the user is comfortable being addressed in. |
| `languages.default` | string |  | Default response language. Um de: `pt-BR`, `en-US`, `es-ES`, `fr-FR`, `de-DE`, `it-IT`, `ja-JP`, `zh-CN`. |
| `languages.switch_on_request` | boolean |  | When true, agent honors mid-session language switches ('fala em inglês'). When false, sticks to default. |
