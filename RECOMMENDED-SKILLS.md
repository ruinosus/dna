...
| **Spec Kit** | [`github/spec-kit`](https://github.com/github/spec-kit) | CLI methodology | DNA's officially supported **spec-driven flow** (`constitution → spec → plan → tasks → implement`). Run it untouched, then `dna specify import` to durably capture the run as DNA Kinds (and `dna specify export` to project them back byte-faithfully). See the [Spec Kit guide](docs/guides/spec-kit.md). | `uvx --from git+https://github.com/github/spec-kit specify init …` |
